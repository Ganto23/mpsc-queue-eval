...
public:
    BasicAtomicQueueAlignas() : _queue(std::make_unique<std::atomic<T>[]>(Capacity)), _capacity(Capacity), _head(0), _tail(0){
...
private:
    alignas(64) std::unique_ptr<std::atomic<T>[]> _queue;
